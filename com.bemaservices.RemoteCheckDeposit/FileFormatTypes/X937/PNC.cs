...
namespace com.bemaservices.RemoteCheckDeposit.FileFormatTypes
{
    ...
    /// DSTU standard.
    /// </summary>
    [Description( "Processes a batch export for PNC." )]
    [Export( typeof( FileFormatTypeComponent ) )]
    [ExportMetadata( "ComponentName", "PNC" )]
    [EncryptedTextField("Origin Routing Number", "Used on Type 10 Record 3 for Account Routing", true, key:"OriginRoutingNumber" )]
    [EncryptedTextField("Payor Routing Number", "Used on Type 61 Record 3 for Account Routing", true, key: "PayorRoutingNumber")]
    [EncryptedTextField( "Destination Routing Number", "", true, key:"DestinationRoutingNumber", defaultValue:"031000053")]
...
Amount: {{ Amount }}", order: 20 )]
    public class PNC : X937DSTU
    {
        ...
        /// repeated. Ever.
        /// </summary>
        protected const string SystemSettingNextCashHeaderId = "PNC.NextCashHeaderId";

        /// <summary>
        /// The system setting that contains the last file modifier we used.
        /// </summary>
        protected const string SystemSettingLastFileModifier = "PNC.LastFileModifier";

        /// <summary>
        /// The last item sequence number used for items.
        /// </summary>
        protected const string LastItemSequenceNumberKey = "PNC.LastItemSequenceNumber";
        ...
        protected override CashLetterControl GetCashLetterControlRecord( ExportOptions options, List<Record> records )
        {
            ...
            control.ItemCount = (records.Where(r => r.RecordType == 25).Count()) + (records.Where(r => r.RecordType == 61).Count());
            //control.ImageCount = control.ImageCount - ( records.Where( r => r.RecordType == 61 ).Count() * 2 ); 
        ...
        {
            var control = base.GetFileControlRecord( options, records );

            control.TotalItemCount = ( records.Where( r => r.RecordType == 25 ).Count() ) + ( records.Where( r => r.RecordType == 61 ).Count() );
        ...
        protected override BundleHeader GetBundleHeader( ExportOptions options, int bundleIndex )
        {
            ...
            header.ReturnLocationRoutingNumber = Rock.Security.Encryption.DecryptString(GetAttributeValue(options.FileFormat, "DestinationRoutingNumber"));
            return header;
        }
        ...
        protected override List<Record> GetItemRecords( ExportOptions options, FinancialTransaction transaction )
        {
            ...
            {
                detailA.BankOfFirstDepositRoutingNumber = Rock.Security.Encryption.DecryptString(GetAttributeValue(options.FileFormat, "DestinationRoutingNumber"));
                ...
                detailA.BankOfFirstDepositAccountNumber = Rock.Security.Encryption.DecryptString( GetAttributeValue( options.FileFormat, "AccountNumber" ) ) + "/";
            }
            //PNC
            foreach( ImageViewDetail detail in records.Where( r => r.RecordType == 50 ).Cast<dynamic>() )
            {
                detail.ImageCreatorRoutingNumber = Rock.Security.Encryption.DecryptString( GetAttributeValue( options.FileFormat, "DestinationRoutingNumber" ) );
            }
